...
export default function PlaygroundsListClient({ org_id, orgslug }: PlaygroundsListClientProps) {
  ...
  const handleCreate = async () => {
    ...
    const name = newName.trim() || t('playgrounds.untitled_playground') || 'Untitled Playground'
    setIsCreating(true)
    ...
    try {
      ...
      toast.error(t('playgrounds.create_error') || 'Failed to create playground')
    } finally {
      setIsCreating(false)
    }
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      clearSelection()
      if (playgroundsKey) mutate(playgroundsKey)
      toast.success(t('playgrounds.deleted_toast', { count: uuids.length }) || `Deleted ${uuids.length} playground(s)`)
    } catch {
      toast.error(t('playgrounds.delete_error') || 'Failed to delete some playgrounds')
    } finally {
  ...
  const handleDuplicate = async () => {
    if (!access_token || selectedUuids.size === 0) return
    const uuids = Array.from(selectedUuids)
    const toastId = toast.loading(t('playgrounds.duplicating_toast', { count: uuids.length }) || `Duplicating ${uuids.length} playground(s)...`)
    try {
      await Promise.all(uuids.map((uuid) => duplicatePlayground(uuid, access_token)))
      if (playgroundsKey) mutate(playgroundsKey)
      toast.success(t('playgrounds.duplicated_toast', { count: uuids.length }) || `Duplicated ${uuids.length} playground(s)`, { id: toastId })
    } catch {
      toast.error(t('playgrounds.duplicate_error') || 'Failed to duplicate some playgrounds', { id: toastId })
    }
  }
  ...
  return (
    <PlanRestrictedFeature
      ...
      descriptionKey="playgrounds.no_playgrounds_description"
    >
      <FeatureDisabledView featureName="playgrounds" orgslug={orgslug} context="dashboard">
        <div className="h-full w-full bg-[#f8f8f8] pl-10 pr-10">
          ...
          <div className="mb-6 pt-6">
            <Breadcrumbs
              items={[{ label: t('playgrounds.playgrounds') || 'Playgrounds', href: '/dash/playgrounds', icon: <Cube size={14} /> }]}
            />
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mt-4">
              <h1 className="text-3xl font-bold mb-4 sm:mb-0">{t('playgrounds.playgrounds') || 'Playgrounds'}</h1>
              ...
              {hasSelection && (
                <div className="flex items-center gap-2 bg-white nice-shadow rounded-lg px-3 py-2">
                  <span className="text-xs font-semibold text-gray-700 pr-2 border-r border-gray-200">
                    {t('boards.selected_count', { count: selectedUuids.size }) || `${selectedUuids.size} selected`}
                  </span>
                  <button
                  ...
                  >
                    ...
                    {t('playgrounds.duplicate') || 'Duplicate'}
                  </button>
                  <button
                  ...
                  >
                    ...
                    {t('playgrounds.download') || 'Download'}
                  </button>
                  <button
              ...
              {filtered.length === 0 && searchQuery && (
                <div className="col-span-full flex justify-center items-center py-8 text-center">
                  <div>
                    ...
                    <h2 className="text-xl font-semibold text-gray-600 mb-2">{t('playgrounds.no_results', { query: searchQuery }) || `No results for "${searchQuery}"`}</h2>
                    <p className="text-gray-400">{t('playgrounds.try_different_search') || 'Try a different search term'}</p>
                  </div>
                </div>
              )}
              ...
              {allPlaygrounds.length === 0 && !searchQuery && (
                <div className="col-span-full flex justify-center items-center py-8 text-center">
                  <div>
                    ...
                    <h2 className="text-2xl font-bold text-gray-600 mb-2">{t('playgrounds.no_playgrounds_yet') || 'No playgrounds yet'}</h2>
                    <p className="text-lg text-gray-400">
                      {t('playgrounds.no_playgrounds_description') || 'Create interactive AI-generated experiences for your learners.'}
                    </p>
                  </div>
                </div>
              )}
          ...
          {totalPages > 1 && (
            <>
              ...
              <div className="mb-6 text-center text-sm text-gray-500">
                {t('pagination.showing_page', { current: currentPage, total: totalPages }) || `Showing page ${currentPage} of ${totalPages}`}
              </div>
            </>
          )}
        </div>
      ...
      {showNameModal && (
        ...
        >
          ...
          >
          <h2 className="text-base font-bold text-gray-900 mb-1">{t('playgrounds.new_playground') || 'New Playground'}</h2>
          <p className="text-xs text-gray-400 mb-4">{t('playgrounds.give_name_description') || 'Give your playground a name to get started.'}</p>
          <input
            autoFocus
            type="text"
            value={newName}
            ...
            placeholder={t('playgrounds.create_placeholder') || 'e.g. Photosynthesis Quiz'}
      ...
      >
        ...
        >
          <h2 className="text-base font-bold text-gray-900 mb-1">
            {t('playgrounds.delete_title', { count: selectedUuids.size }) || `Delete ${selectedUuids.size} playgrounds`}
          </h2>
          <p className="text-sm text-gray-500 mb-5">
            {t('playgrounds.delete_confirm') || 'This action is permanent and cannot be undone.'}
          </p>
          <div className="flex gap-2 justify-end">
            <button
              onClick={() => setShowDeleteConfirm(false)}
...
function PlaygroundCard({
  ...
  const { t } = useTranslation()
  const accessBadge = {
    public: { icon: <Globe size={10} />, label: t('playgrounds.access.public') || 'Public', className: 'bg-green-100 text-green-700' },
    authenticated: { icon: <Users size={10} />, label: t('playgrounds.access.members') || 'Members', className: 'bg-blue-100 text-blue-700' },
    restricted: { icon: <Lock size={10} />, label: t('playgrounds.access.restricted') || 'Restricted', className: 'bg-amber-100 text-amber-700' },
  }[playground.access_type as string] ?? { icon: <Lock size={10} />, label: t('playgrounds.access.private') || 'Private', className: 'bg-gray-100 text-gray-600' }
        ...
        <div className="absolute bottom-2 left-2 flex items-center gap-1.5">
          ...
          {!playground.published && (
            <span className="flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide bg-gray-100 text-gray-500 rounded-full">
              {t('playgrounds.access.draft') || 'Draft'}
            </span>
          )}
        </div>
      </div>
