...
import { ImportTypeSelector, Int42hCourseImport } from '@components/Objects/Modals/Course/Import'
...
function CoursesHome(params: CourseProps) {
  ...
  const [importType, setImportType] = React.useState<'select' | 'scorm' | 'int42h'>('select')
  ...
  const handleImportTypeSelect = (type: 'scorm' | 'int42h') => {
    setImportType(type)
  }
  ...
  const getImportModalTitle = () => {
    switch (importType) {
      ...
      case 'int42h':
        return t('dashboard.courses.import_int42h')
      default:
  ...
  const getImportModalDescription = () => {
    switch (importType) {
      ...
      case 'int42h':
        return t('dashboard.courses.import_int42h_description')
      default:
  ...
  const bulkExportCourses = async () => {
    ...
    try {
      ...
      downloadBlob(blob, `int42h-courses-export-${timestamp}.zip`)
