...
function EmbedActivityClient({ activity, course, activityId, orgslug }: EmbedActivityClientProps) {
  ...
  const showInt42hLogo = searchParams.get('showint42hlogo') !== 'false' && searchParams.get('showint42hlogo') !== 'false'
  ...
  if (!isEmbeddable) {
    return (
      <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center p-8">
        ...
        {showInt42hLogo && <PoweredByBadge activityUrl={getActivityUrl()} />}
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen relative" style={customStyles}>
      ...
      {showInt42hLogo && ready && <PoweredByBadge activityUrl={getActivityUrl()} />}
    </div>
  )
}
