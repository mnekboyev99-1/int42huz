...
import { getINT42H_TOP_DOMAIN_VAL, getDeploymentMode } from '@services/config/config'
...
const LoginClient = (props: LoginClientProps) => {
  ...
  const handleGoogleSignIn = () => {
    ...
    if (props.org?.slug) {
      const topDomain = getINT42H_TOP_DOMAIN_VAL();
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, {validateForm, setErrors, setSubmitting}) => {
      ...
      if (res && res.error) {
        ...
        // First signIn already authenticated and set cookies вЂ” just redirect
              ...
              <div className="flex-1 min-w-0">
                ...
                {errorType === 'ACCOUNT_LOCKED' && (
                  <span className="text-sm font-medium">
                    ...
                    {retryAfter ? ` В· ${t('auth.try_again_in', { minutes: Math.max(1, Math.ceil(retryAfter / 60)) })}` : ''}
                  </span>
                )}
