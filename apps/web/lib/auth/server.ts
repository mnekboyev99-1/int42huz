...
const getBackendUrlVal = () => getBackendUrl().replace(/\/+$/, '')
...
export async function getServerSession(): Promise<Session | null> {
  try {
    ...
    if (accessToken?.value) {
      ...
      const sessionResponse = await fetch(`${getBackendUrlVal()}/api/v1/users/session`, {
        method: 'GET',
        headers: {
    ...
    const refreshResponse = await fetch(`${getBackendUrlVal()}/api/v1/auth/refresh`, {
      method: 'GET',
      headers: {
...
export async function getServerAccessToken(): Promise<string | null> {
  try {
    ...
    const response = await fetch(`${getBackendUrlVal()}/api/v1/auth/refresh`, {
      method: 'GET',
      headers: {
        Cookie: `${REFRESH_TOKEN_COOKIE}=${refreshToken.value}`,
      },
