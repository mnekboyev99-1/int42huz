...
function getMediaUrl() {
  const mediaUrl = getConfig('NEXT_PUBLIC_INT42H_MEDIA_URL');
  if (mediaUrl) {
    return mediaUrl;
  } else {
...
export function getOrgFontMediaDirectory(orgUUID: string, fileId: string) {
  return `${getMediaUrl()}content/orgs/${orgUUID}/fonts/${fileId}`
}
