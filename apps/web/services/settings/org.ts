...
export async function uploadOrgFont(
  ...
  formData.append('font_file', font_file)
  const result: any = await fetch(
    `${getAPIUrl()}orgs/${org_id}/fonts`,
...
export async function deleteOrgFont(
  org_id: string,
  font_name: string,
  access_token: string
) {
  const result: any = await fetch(
    `${getAPIUrl()}orgs/${org_id}/fonts/${encodeURIComponent(font_name)}`,
