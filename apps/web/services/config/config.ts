...
function loadRuntimeConfig(): Record<string, string> {
  ...
  if (typeof window === 'undefined') {
    ...
    try {
      ...
      const possiblePaths = [
        path.join(/*turbopackIgnore: true*/ process.cwd(), 'runtime-config.json'),
        path.join(/*turbopackIgnore: true*/ __dirname || process.cwd(), 'runtime-config.json'),
        path.join(/*turbopackIgnore: true*/ __dirname || process.cwd(), '..', 'runtime-config.json'),
      ];
...
const getINT42H_HTTP_PROTOCOL = () =>
  (getConfig('NEXT_PUBLIC_INT42H_HTTPS') === 'true') ? 'https://' : 'http://'
const getINT42H_BACKEND_URL = () => getConfig('NEXT_PUBLIC_INT42H_BACKEND_URL', 'http://localhost/')
const getINT42H_DOMAIN = () => {
  // 1. Env var (backward compat for existing deploys)
  const envVal = getConfig('NEXT_PUBLIC_INT42H_DOMAIN')
...
const getINT42H_TOP_DOMAIN = () => {
  // 1. Env var (backward compat for existing deploys)
  const envVal = getConfig('NEXT_PUBLIC_INT42H_TOP_DOMAIN')
  if (envVal) return envVal
  // 2. Cookie set by middleware from backend instance info
  const cookieVal = getCookieValue('LH_top_domain')
  if (cookieVal) return cookieVal
  // 3. Derive from DOMAIN by stripping port
  const domain = getINT42H_DOMAIN()
  return domain.split(':')[0]
}
const getINT42H_TELEMETRY_DISABLED = () => getConfig('NEXT_TELEMETRY_DISABLED', 'true').toLowerCase();
const getINT42H_PLATFORM_URL = (): string | null => {
  // NEXT_PUBLIC_ variant (available client-side via runtime config)
  const pubVal = getConfig('NEXT_PUBLIC_INT42H_PLATFORM_URL')
  if (pubVal) return pubVal.replace(/\/+$/, '')
  // Non-prefixed variant (server-side only, backward compat)
  const val = getConfig('INT42H_PLATFORM_URL')
  if (val) return val.replace(/\/+$/, '')
  return null
}

// Export getter functions for dynamic runtime configuration
export const getINT42H_HTTP_PROTOCOL_VAL = getINT42H_HTTP_PROTOCOL
export const getINT42H_BACKEND_URL_VAL = getINT42H_BACKEND_URL
export const getINT42H_DOMAIN_VAL = getINT42H_DOMAIN
export const getINT42H_TOP_DOMAIN_VAL = getINT42H_TOP_DOMAIN
export const getINT42H_TELEMETRY_DISABLED_VAL = getINT42H_TELEMETRY_DISABLED
export const getINT42H_PLATFORM_URL_VAL = getINT42H_PLATFORM_URL
...
export const INT42H_HTTP_PROTOCOL = getINT42H_HTTP_PROTOCOL()
export const INT42H_BACKEND_URL = getINT42H_BACKEND_URL()
export const INT42H_DOMAIN = getINT42H_DOMAIN()
export const INT42H_TOP_DOMAIN = getINT42H_TOP_DOMAIN()
...
// Derive API URL from backend URL (with backward compat for NEXT_PUBLIC_INT42H_API_URL)
const deriveAPIUrl = (): string => {
  // Backward compat: if explicit API URL is set, use it
  const explicitApiUrl = getConfig('NEXT_PUBLIC_INT42H_API_URL')
  if (explicitApiUrl) return explicitApiUrl
  // Derive from backend URL
  const backendUrl = getINT42H_BACKEND_URL().replace(/\/+$/, '')
...
export const getBackendUrl = () => getINT42H_BACKEND_URL()

/**
 * Get the upgrade/plan URL for a given org.
 * Uses INT42H_PLATFORM_URL (the main platform, e.g. int42h.app).
...
export const getUpgradeUrl = (orgSlug: string): string | null => {
  ...
  const platformUrl = getINT42H_PLATFORM_URL()
  if (!platformUrl) return null
  return `${platformUrl}/dashboard/${orgSlug}/plan`
}

/**
 * Build a URL on the platform domain (e.g. int42h.app).
...
//
// We deliberately do NOT consult `NEXT_PUBLIC_INT42H_MULTI_ORG` here —
...
export const getCustomDomainFromContext = (): string | null => {
  if (typeof window !== 'undefined') {
    ...
    // Check if current hostname is a custom domain (not a subdomain of INT42H_DOMAIN)
...
export const getUriWithOrg = (orgslug: string, path: string) => {
  ...
  if (typeof window !== 'undefined') {
    ...
    const baseDomain = stripPort(getINT42H_DOMAIN())
  ...
  if (orgslug) {
    const protocol = getINT42H_HTTP_PROTOCOL()
    const domain = getINT42H_DOMAIN()
    return `${protocol}${orgslug}.${domain}${path}`
  }
  const explicitDomain = getConfig('NEXT_PUBLIC_INT42H_DOMAIN')
  if (explicitDomain) {
    const protocol = getINT42H_HTTP_PROTOCOL()
...
export const getDefaultOrg = () => {
  ...
  const envVal = getConfig('NEXT_PUBLIC_INT42H_DEFAULT_ORG')
  if (envVal) return envVal
