...
const OrgScripts: React.FC = () => {
  ...
  const loadScript = (scriptContent: string, scriptName: string) => {
    try {
      ...
      const safeScriptId = `int42h-org-script-${scriptName.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-${Math.random().toString(36).substr(2, 9)}`
      ...
      if (scriptContent.trim().toLowerCase().startsWith('<script')) {
        ...
        if (scriptTag.src) {
          ...
          scriptElement.textContent = `
            /* Int42h Organization Script - ${scriptName} */
            try {
              (function() {
                'use strict';
                ${sanitizedContent}
        ...
        const comment = document.createComment(` Int42h Organization Script - ${scriptName} (${safeScriptId}) `)
  ...
  useEffect(() => {
    ...
    return () => {
      const scripts = document.querySelectorAll('script[id^="int42h-org-script-"]')
