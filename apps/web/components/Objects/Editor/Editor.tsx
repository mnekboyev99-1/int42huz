...
import int42hAI_icon from 'public/int42h_ai_simple.png'
import {
  AIEditorStateTypes,
  useAIEditor,
  useAIEditorDispatch,
...
import SliderBlock from './Extensions/Slider/SliderBlock'
import Youtube from '@tiptap/extension-youtube'
import VideoBlock from './Extensions/Video/VideoBlock'
import AudioBlock from './Extensions/Audio/AudioBlock'
import { Eye, Monitor, History, AlertTriangle, RefreshCw, GitMerge, Loader2, Merge, Split, Trash2, ChevronDown, Plus, Palette } from 'lucide-react'
import MathEquationBlock from './Extensions/MathEquation/MathEquationBlock'
import PDFBlock from './Extensions/PDF/PDFBlock'
import AttachmentBlock from './Extensions/Attachment/AttachmentBlock'
...
import { LineHeight } from './Extensions/FontSize/LineHeight'
import { LetterSpacing } from './Extensions/FontSize/LetterSpacing'
import { Indent } from './Extensions/FontSize/Indent'
import { Details, DetailsSummary, DetailsContent } from './Extensions/Callout/Details'
import { Columns, Column } from './Extensions/EmbedObjects/Columns'
...
export function Editor(props: EditorProps) {
  ...
  const extensions = React.useMemo(
    () => [
      ...
      SliderBlock.configure({ editable: true, activity: stableActivity }),
      ...
      AttachmentBlock.configure({ editable: true, activity: stableActivity }),
  ...
  return (
    <div className="activity-editor-page">
      ...
      <CourseProvider courseuuid={props.course.course_uuid} initialCourseStructure={props.course}>
          <div className="activity-editor-top">
            <div className="activity-editor-doc-section">
              ...
              <div className="activity-editor-buttons-wrapper">
                <ToolbarButtons editor={editor} props={props} />
              </div>
            </div>
        ...
        >
          <div className="activity-editor-content-wrapper" style={{ flex: 1, margin: 0, marginTop: '155px', display: 'flex', flexDirection: 'column' }}>
            ...
            {editor && (
              ...
              >
                <div className="flex items-center gap-0.5 p-1 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl shadow-xl z-50 overflow-visible">
                  {/* Merge Cells */}
                  <button
                    type="button"
                  ...
                  >
                    <div className="relative flex items-center justify-center">
                      <Plus size={10} className="absolute -top-1" />
                      <span className="text-[9px] font-bold mt-2">Row</span>
                    </div>
                  </button>
                  ...
                  >
                    <div className="relative flex items-center justify-center">
                      <Plus size={10} className="absolute -bottom-1" />
                      <span className="text-[9px] font-bold mb-2">Row</span>
                    </div>
                  </button>
                  ...
                  >
                    <div className="relative flex items-center justify-center">
                      <Plus size={10} className="absolute -left-1" />
                      <span className="text-[9px] font-bold ml-2">Col</span>
                    </div>
                  </button>
                  ...
                  >
                    <div className="relative flex items-center justify-center">
                      <Plus size={10} className="absolute -right-1" />
                      <span className="text-[9px] font-bold mr-2">Col</span>
                    </div>
                  </button>
                  ...
                  <div className="relative group overflow-visible">
                    <button
                      type="button"
                      className="p-1.5 hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded-lg text-neutral-600 dark:text-neutral-300 transition-colors flex items-center gap-0.5"
                      title="Cell Background Color"
                    >
                      <Palette size={15} />
                      <ChevronDown size={10} />
                    </button>
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:grid grid-cols-5 gap-1 p-2 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl shadow-xl min-w-[130px] z-[999]">
                      {[
                        { color: null, bg: '#ffffff', border: '#e5e7eb', title: 'No Color' },
                        { color: '#f3f4f6', bg: '#f3f4f6', title: 'Gray' },
                        { color: '#fee2e2', bg: '#fee2e2', title: 'Red' },
                        { color: '#ffedd5', bg: '#ffedd5', title: 'Orange' },
                        { color: '#fef3c7', bg: '#fef3c7', title: 'Yellow' },
                        { color: '#dcfce7', bg: '#dcfce7', title: 'Green' },
                        { color: '#ccfbf1', bg: '#ccfbf1', title: 'Teal' },
                        { color: '#e0f2fe', bg: '#e0f2fe', title: 'Sky' },
                        { color: '#dbeafe', bg: '#dbeafe', title: 'Blue' },
                        { color: '#f3e8ff', bg: '#f3e8ff', title: 'Purple' },
                      ].map((c, i) => (
                        <div
                          key={i}
                          onClick={() => editor.chain().focus().setCellAttribute('background', c.color).run()}
                  ...
                  <button
                    ...
                    className="p-1.5 hover:bg-red-50 dark:hover:bg-red-950/20 text-neutral-500 hover:text-red-600 rounded-lg transition-colors flex flex-col items-center"
                    title="Delete Row"
                  >
                    <Trash2 size={13} />
                    <span className="text-[8px] font-bold block leading-none mt-0.5">Row</span>
                  </button>
                  ...
                  >
                    <Trash2 size={13} />
                    <span className="text-[8px] font-bold block leading-none mt-0.5">Col</span>
                  </button>
                  ...
                  >
                    <Trash2 size={13} />
                    <span className="text-[8px] font-bold block leading-none mt-0.5">Tab</span>
                  </button>
                </div>
              </BubbleMenu>
            )}
            {editor?.storage?.characterCount && (
              <div className="flex items-center justify-end gap-4 px-4 py-2 mt-2 bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-lg text-xs text-neutral-500 dark:text-neutral-400 font-medium select-none">
                <span>Words: {editor.storage.characterCount.words()}</span>
                <span>Characters: {editor.storage.characterCount.characters()}</span>
                <span>Reading Time: {Math.max(1, Math.round(editor.storage.characterCount.words() / 200))} min</span>
              </div>
            )}
          </div>
