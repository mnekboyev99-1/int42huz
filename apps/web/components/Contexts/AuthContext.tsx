...
import {
  getAPIUrl,
  getINT42H_TOP_DOMAIN_VAL,
  getINT42H_DOMAIN_VAL,
...
const AUTH_BROADCAST_CHANNEL = 'int42h_auth_sync'
...
function isCustomDomain(): boolean {
  ...
  const domain = getINT42H_DOMAIN_VAL()
    ...
    async (provider: string, options: SignInOptions = {}): Promise<SignInResult | void> => {
      ...
      try {
        ...
        if (provider === 'google') {
          ...
          const redirectUri = `${window.location.protocol}//${getINT42H_DOMAIN_VAL()}/auth/callback/google`
