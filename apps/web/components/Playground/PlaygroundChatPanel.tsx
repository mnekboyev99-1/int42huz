      ...
      <div className="px-4 py-3 border-b border-gray-100 flex-shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-neutral-700">{t('playgrounds.chat.ai_generator') || 'AI Generator'}</span>
          </div>
        ...
        {orgCourses.length > 0 && !sessionStarted && (
          <div className="mt-3 space-y-2">
            <p className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider">{t('playgrounds.chat.source') || 'Source'}</p>
            <div className="flex gap-1.5">
              ...
              >
                ...
                {t('playgrounds.chat.ai_knowledge') || 'AI Knowledge'}
              </button>
              <button
              ...
              >
                ...
                {t('playgrounds.chat.course') || 'Course'}
              </button>
            </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-0">
        {messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center px-4 gap-2">
            ...
            <p className="text-sm font-semibold text-neutral-600">{t('playgrounds.chat.describe_what_to_create') || 'Describe what to create'}</p>
            <p className="text-xs text-neutral-400">
              {sourceMode === 'course'
                ? (t('playgrounds.chat.course_grounded_desc') || 'Content will be grounded in your course material')
                : (t('playgrounds.chat.prompt_suggestion_desc') || 'Type a prompt or pick a suggestion below')
              }
            </p>
          </div>
        ) : (
              ...
              >
                {msg.role === 'model' ? (
                  <span className="text-xs italic flex items-center gap-1.5">
                    ...
                    {t('playgrounds.chat.generated_chars', { count: msg.content.length }) || `Generated (${msg.content.length.toLocaleString()} chars)`}
                  </span>
                ) : (
                  msg.content
                )}
        ...
        {isGenerating && (
          <div className="flex justify-start">
            <div className="bg-neutral-100 nice-shadow px-3 py-2 rounded-xl flex items-center gap-2">
              ...
              <span className="text-xs text-neutral-500 font-medium">{t('playgrounds.chat.generating_loading') || 'Generating…'}</span>
            </div>
          </div>
        )}
      ...
      <div className="p-3 border-t border-gray-100 flex-shrink-0">
        ...
        >
          <textarea
            ...
            placeholder={
              remaining === 0
                ? (t('playgrounds.chat.max_iterations_reached') || 'Max iterations reached')
                : iterationCount === 0
                ? (t('playgrounds.chat.input_placeholder_initial') || 'Describe your interactive experience…')
                : (t('playgrounds.chat.input_placeholder_iterate') || 'Request a change…')
            }
          ...
          <div className="flex items-center justify-between px-3 pb-2">
            <span className="text-[10px] text-neutral-400 font-medium">
              {remaining === 0 ? (
                <span className="text-red-400">{t('playgrounds.chat.no_generations_left') || 'No generations left'}</span>
              ) : (
                t('playgrounds.chat.input_hint') || 'Enter to send · Shift+Enter for newline'
              )}
            </span>
            <button
              onClick={handleSend}
            ...
            >
              ...
              {isGenerating ? (t('playgrounds.chat.generating') || 'Generating') : (t('playgrounds.chat.send') || 'Send')}
            </button>
          </div>
        </div>
      </div>
