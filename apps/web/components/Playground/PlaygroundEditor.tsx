...
const EditorInt42hLogo = () => {
  ...
  return (
    <div className="bg-black rounded-md w-[25px] h-[25px] flex items-center justify-center overflow-hidden flex-shrink-0">
      ...
      >
        <Image src="/lrn.svg" alt="Int42h" width={14} height={14} className="invert" />
      </motion.div>
    </div>
  )
}
    ...
    <div className="activity-editor-page flex flex-col" style={{ paddingTop: 0 }}>
      ...
      >
        ...
        >
          ...
          <span>{t('common.playgrounds') || 'Playgrounds'}</span>
        </Link>
        ...
        <div className="flex-1 flex items-center gap-1.5 min-w-0 group">
          <div className={`flex-1 flex items-center gap-1.5 px-2 py-1 rounded-lg transition-all min-w-0 ${
            ...
            <input
              ...
              placeholder={t('playgrounds.editor.title_placeholder') || 'Playground title'}
            />
          ...
          {titleSaveStatus === 'saved' && !isSavingTitle && (
            <span className="flex items-center gap-0.5 text-[10px] font-semibold text-green-500 flex-shrink-0">
              ...
              {t('playgrounds.editor.saved') || 'Saved'}
            </span>
          )}
          {titleDirty && !titleFocused && !isSavingTitle && titleSaveStatus === 'idle' && (
            <span className="w-1.5 h-1.5 rounded-full bg-amber-400 flex-shrink-0" title={t('playgrounds.editor.unsaved_name_change') || 'Unsaved name change'} />
          )}
        </div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          >
            ...
            {t('playgrounds.editor.options') || 'Options'}
          </button>
          ...
          >
            {saveStatus === 'saved' ? (
              <><Check size={14} weight="bold" />{t('playgrounds.editor.saved') || 'Saved'}</>
            ) : (
              <><FloppyDisk size={14} weight="bold" />{t('playgrounds.editor.save') || 'Save'}</>
            )}
          </button>
          ...
          >
            {playground.published ? (
              <><Eye size={14} weight="bold" />{t('playgrounds.editor.published') || 'Published'}</>
            ) : (
              <><EyeSlash size={14} weight="bold" />{t('playgrounds.editor.publish') || 'Publish'}</>
            )}
          </button>
          ...
          >
            ...
            {t('playgrounds.editor.preview') || 'Preview'}
          </Link>
        </div>
      </div>
