...
Int42h supports two tenancy modes, selected by a single environment variable: `INT42H_TENANCY`.
...
```
INT42H_TENANCY=multi   # requires EE (or SaaS) + INT42H_DOMAIN
INT42H_TENANCY=single  # default for OSS / dev / VPS self-host
# unset                    # inferred — see "Default inference" below
```

The backend validates at boot:
- `multi` requires `is_ee_available()` OR `INT42H_SAAS=true`, AND `INT42H_DOMAIN` set to a routable parent domain (not `localhost`). Mis-configuration aborts startup with a clear error.
- `single` ignores `INT42H_DOMAIN` and `INT42H_COOKIE_DOMAIN`. If you set them, you'll see a deprecation warning at boot — they are no longer authoritative.

### Default inference (when `INT42H_TENANCY` is unset)
...
|----------------------------------------------------------------------------------------------|------------------|
| `INT42H_SAAS=true` + real (non-localhost) domain + not self-hosted/dev                   | `multi`          |
| EE folder present + `INT42H_COOKIE_DOMAIN` starts with `.` + real domain + not self/dev  | `multi`          |
| Anything else                                                                                | `single`         |

The strong signal for "I want subdomain tenancy" is a dotted cookie domain (`INT42H_COOKIE_DOMAIN=.foo.com`) — operators only set that when they want subdomains to share auth. This means an EE deployment on a single VPS domain (no dotted cookie domain) defaults to `single`, which is the right behavior for the self-host-EE case (one org, EE features locally available).

**Recommended:** set `INT42H_TENANCY` explicitly in production. Inference is a backward-compat convenience, not a long-term contract.

## Compatibility with deployment modes (oss / ee / saas)

`INT42H_TENANCY` is orthogonal to the existing **deployment mode** (`oss`, `ee`, `saas`) which is set by the presence of the `ee/` folder and `INT42H_SAAS`. The matrix:
...
Existing EE/SaaS deployments that haven't set `INT42H_TENANCY` continue to behave as before because the inference reads their existing config (`INT42H_SAAS`, `INT42H_COOKIE_DOMAIN`, `INT42H_DOMAIN`) and picks `multi` when those signal subdomain intent.

## Mode 1 — Multi (EE)

Required env:
- `INT42H_TENANCY=multi`
- `INT42H_DOMAIN=your-domain.tld` (the apex; e.g. `int42h.io`)
- `INT42H_COOKIE_DOMAIN=.your-domain.tld` (so subdomains share the session)
- `ee/` folder present (or unset `INT42H_DISABLE_EE`)
...
Required env:
- `INT42H_TENANCY=single` (or just leave unset; this is the default)
...
All Int42h cookies use the `LH_` prefix.
...
|------------|----------------------------------------------------------------------------|----------------------------------------------|
| `multi`    | Configured via `INT42H_ALLOWED_REGEXP` (matches apex + subdomains)     | Strict allowlist for known multi-tenant domain |
...
```json
{
  ...
  "frontend_domain": "int42h.io",
  "top_domain": "int42h.io"
}
```
...
`_is_allowed_base_url` validates the URL against an allowlist before returning it. In `single` mode the operator's host is authoritative — any well-formed http(s) URL is accepted. In `multi` mode, the URL must match `INT42H_ALLOWED_ORIGINS` / `INT42H_ALLOWED_REGEXP` / `INT42H_PLATFORM_URL`.

## EE / OSS code split

The OSS frontend never imports subdomain or custom-domain logic directly. All of that lives under `int42h/apps/web/ee/services/tenancy/`:
...
The OSS `proxy.ts` and `services/org/orgResolution.ts` reach these via dynamic `import()` wrapped in try/catch. If EE is unavailable (folder removed at deploy time, or `INT42H_DISABLE_EE=1`) the resolver falls back to the default org and logs a warning.
...
```sh
INT42H_TENANCY=multi
INT42H_DOMAIN=lh.test
INT42H_COOKIE_DOMAIN=.lh.test
```
Add hosts entries:
```
...
```sh
INT42H_DISABLE_EE=1
INT42H_TENANCY=multi   # should fail at boot
INT42H_TENANCY=single  # should boot cleanly
```
With `single`, no EE imports are attempted at runtime.

## Upgrading from versions before `INT42H_TENANCY` existed

Prior versions decided multi-vs-single behavior implicitly from `is_multi_org_allowed()` (deployment mode in `ee`/`saas`), `self_hosted`, `development_mode`, and `INT42H_USE_DEFAULT_ORG`. After the introduction of `INT42H_TENANCY` these flags are deprecated and the new env var is the source of truth.
...
1. Check the boot log for any deprecation warnings about `INT42H_USE_DEFAULT_ORG`, `INT42H_SELF_HOSTED`, or `INT42H_COOKIE_DOMAIN` being set in single mode.
2. Hit `GET /api/v1/instance/info` and confirm `tenancy` matches your expectation.
3. Set `INT42H_TENANCY` explicitly in your env config to lock the behavior — inference is a backward-compat convenience, not a long-term contract.
...
2. **`multi` mode** requires:
   - EE folder included in the build (`docker build --build-arg INT42H_PUBLIC=false`)
   ...
   - Backend env: `INT42H_TENANCY=multi`, `INT42H_DOMAIN`, `INT42H_COOKIE_DOMAIN`, `INT42H_ALLOWED_REGEXP`
3. **`single` mode** on a VPS:
   - Point any domain at the server. No env config beyond `INT42H_TENANCY=single` is needed.
...
Backend:
- `int42h/apps/api/config/config.py` — `INT42H_TENANCY` env, validation
- `int42h/apps/api/src/routers/instance.py` — `instance/info` endpoint
- `int42h/apps/api/src/routers/auth.py` — `get_cookie_domain_for_request`
- `int42h/apps/api/src/services/email/utils.py` — `get_org_signup_base_url`
- `int42h/apps/api/app.py` — CORS branching
- `int42h/apps/api/src/core/ee_hooks.py` — `is_multi_org_allowed`, EE detection

Frontend:
- `int42h/apps/web/proxy.ts` — middleware (resolveTenant)
- `int42h/apps/web/ee/services/tenancy/core.ts` — pure resolver logic
- `int42h/apps/web/ee/services/tenancy/resolveMulti.middleware.ts` — Edge entry
- `int42h/apps/web/ee/services/tenancy/resolveMulti.server.ts` — Node entry
- `int42h/apps/web/services/org/orgResolution.ts` — server-side org resolution
- `int42h/apps/web/services/auth/cookies.ts` — cookie options
- `int42h/apps/web/services/config/config.ts` — `getTenancy`, `getUriWithOrg`
